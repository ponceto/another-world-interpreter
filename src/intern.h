...
public: // public interface
   ...
   ~Ptr() = default;
    ...
    auto advance(uint32_t offset) -> Ptr&
    {
        if(_bufptr != nullptr) {
            _bufptr += offset;
        }
    ...
    auto rewind(uint32_t offset) -> Ptr&
    {
        if(_bufptr != nullptr) {
            _bufptr -= offset;
        }

...
class FileStdcImpl final
    : public FileImpl
{
public: // public interface
    FileStdcImpl();

    FileStdcImpl(FileStdcImpl&&) = delete;

    FileStdcImpl(const FileStdcImpl&) = delete;

    FileStdcImpl& operator=(FileStdcImpl&&) = delete;

    FileStdcImpl& operator=(const FileStdcImpl&) = delete;

    virtual ~FileStdcImpl();
...
{
    if(!_impl && (impl == "stdc")) {
        _impl = std::make_unique<FileStdcImpl>();
    }
    if(!_impl && (impl == "zlib")) {
...
auto FileStdcImpl::open(const std::string& path, const std::string& mode) -> bool
{
    if(_file != nullptr) {
...
auto FileStdcImpl::seek(int32_t offset) -> bool
{
    if(_file != nullptr) {
...
auto FileStdcImpl::read(void* buffer, uint32_t length) -> bool
{
    if(_file != nullptr) {
...
auto FileStdcImpl::write(void* buffer, uint32_t length) -> bool
{
    if(_file != nullptr) {

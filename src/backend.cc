...
SDL::SDL()
    ...
    , _flags(0)
{
    log_debug(SYS_BACKEND, "starting sdl...");
    if(_status == 0) {
    ...
    log_debug(SYS_BACKEND, "sdl is started!");
}

SDL::~SDL()
{
    log_debug(SYS_BACKEND, "stopping sdl...");
    if(_status == 0) {
        ::SDL_Quit();
    }
    log_debug(SYS_BACKEND, "sdl is stopped!");
}
...
{
    log_debug(SYS_BACKEND, "starting...");
    if(bool(_sdl) == false) {
        _sdl = std::make_unique<SDL>();
    }
    realize();
    log_debug(SYS_BACKEND, "started!");
}

auto BackendImpl::reset() -> void
{
    log_debug(SYS_BACKEND, "resetting...");
    log_debug(SYS_BACKEND, "reset!");
}

auto BackendImpl::stop() -> void
{
    log_debug(SYS_BACKEND, "stopping...");
    unrealize();
    if(bool(_sdl) != false) {
        _sdl.reset();
    }
    log_debug(SYS_BACKEND, "stopped!");
}
...
auto BackendImpl::updateScreen(const Page& page, const Palette& palette) -> void
{
    ...
    /* compute fps */ {
        ...
        if(_currTicks >= _nextTicks) {
            ...
            if(dt != 0) {
                log_debug(SYS_BACKEND, "framerate: %dfps", ((_frames * 1000) / dt));
            }
...
{
    log_debug(SYS_BACKEND, "starting audio...");
    ...
    log_debug(SYS_BACKEND, "audio started!");
}

auto BackendImpl::stopAudio() -> void
{
    log_debug(SYS_BACKEND, "stopping audio...");
    ::SDL_CloseAudio();
    log_debug(SYS_BACKEND, "audio stopped!");
}
...
auto BackendImpl::realize() -> void
{
    ...
    {
        log_debug(SYS_BACKEND, "realizing...");
        set_hints();
        create_window();
        create_renderer();
        create_texture();
        log_debug(SYS_BACKEND, "realized!");
    };
...
auto BackendImpl::unrealize() -> void
{
    ...
    {
        log_debug(SYS_BACKEND, "unrealizing...");
        destroy_texture();
        destroy_renderer();
        destroy_window();
        log_debug(SYS_BACKEND, "unrealized!");
    };

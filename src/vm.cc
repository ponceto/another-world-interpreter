...
{
    log_debug(SYS_VM, "starting...");
    log_debug(SYS_VM, "started!");
}

auto VirtualMachine::reset() -> void
{
    log_debug(SYS_VM, "resetting...");
...
#endif
    log_debug(SYS_VM, "reset!");
}

auto VirtualMachine::stop() -> void
{
    log_debug(SYS_VM, "stopping...");
    log_debug(SYS_VM, "stopped!");
}
...
auto VirtualMachine::run(const Controls& controls) -> void
{
    auto is_ready = [&]() -> bool
    {
    ...
    auto schedule_next_frame = [&]() -> void
    {
        _currTicks = _engine.getTicks();
        const uint32_t elapsed = _currTicks - _prevTicks;
        const uint32_t delay   = _registers[VM_VARIABLE_PAUSE_SLICES].u * 20;

        if(delay > elapsed) {
            _nextTicks = _currTicks + delay - elapsed;
        }
        else {
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],[$%02x]", thread.thread_id, thread.current_pc, "movr", dst, src);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],$%04x", thread.thread_id, thread.current_pc, "movi", dst, imm);
#endif
...
auto VirtualMachine::op_addr(Thread& thread) -> void
{
    ...
    const uint8_t src = _bytecode.fetchByte();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],[$%02x]", thread.thread_id, thread.current_pc, "addr", dst, src);
#endif
    _registers[dst].u += _registers[src].u;
}
...
auto VirtualMachine::op_addi(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],$%04x", thread.thread_id, thread.current_pc, "addi", dst, imm);
#endif
    _registers[dst].u += imm;
}
...
auto VirtualMachine::op_subr(Thread& thread) -> void
{
    ...
    const uint8_t src = _bytecode.fetchByte();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],[$%02x]", thread.thread_id, thread.current_pc, "subr", dst, src);
#endif
    _registers[dst].u -= _registers[src].u;
}
...
auto VirtualMachine::op_andi(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],$%02x", thread.thread_id, thread.current_pc, "andi", dst, imm);
#endif
    _registers[dst].u &= imm;
}
...
auto VirtualMachine::op_iori(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],$%02x", thread.thread_id, thread.current_pc, "iori", dst, imm);
#endif
    _registers[dst].u |= imm;
}
...
auto VirtualMachine::op_shli(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],%d", thread.thread_id, thread.current_pc, "shli", dst, imm);
#endif
    _registers[dst].u <<= imm;
}
...
auto VirtualMachine::op_shri(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],%d", thread.thread_id, thread.current_pc, "shri", dst, imm);
#endif
    _registers[dst].u >>= imm;
}
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%04x", thread.thread_id, thread.current_pc, "jump", loc);
#endif
    _bytecode.seek(loc);
}
...
auto VirtualMachine::op_cjmp(Thread& thread) -> void
{
    ...
    if(variant & 0x80) {
...
#ifndef NDEBUG
        log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s %s,[$%02x],[$%02x],$%04x", thread.thread_id, thread.current_pc, "jump", condition[compare], rg1, rg2, loc);
#endif
    }
        ...
        loc = _bytecode.fetchWord();
#ifndef NDEBUG
        log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s %s,[$%02x],$%04x,$%04x", thread.thread_id, thread.current_pc, "jump", condition[compare], rg1, op2, loc);
#endif
    }
    else {
        ...
        loc = _bytecode.fetchWord();
#ifndef NDEBUG
        log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s %s,[$%02x],$%02x,$%04x", thread.thread_id, thread.current_pc, "jump", condition[compare], rg1, op2, loc);
#endif
    }
#ifdef BYPASS_PROTECTION
    if((compare == 0) && (rg1 == 0x29) && (rg2 == 0x1e)) {
        if(_engine.getCurPartId() == GAME_PART0) {
            log_debug(SYS_VM, "bypassing protection...");
            // 4 symbols
...
auto VirtualMachine::op_djnz(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s [$%02x],$%04x", thread.thread_id, thread.current_pc, "djnz", dst, imm);
#endif
...
auto VirtualMachine::op_call(Thread& thread) -> void
{
    ...
    const uint16_t loc = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%04x", thread.thread_id, thread.current_pc, "call", loc);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s (return to caller)", thread.thread_id, thread.current_pc, "ret");
#endif
    _bytecode.seek(loc);
}
...
auto VirtualMachine::op_init(Thread& thread) -> void
{
    ...
    const uint16_t imm = _bytecode.fetchWord();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,$%04x", thread.thread_id, thread.current_pc, "init", tid, imm);
#endif
...
{
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s (kill this thread)", thread.thread_id, thread.current_pc, "kill");
#endif
...
{
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s (goto next thread)", thread.thread_id, thread.current_pc, "yield");
#endif
    thread.yield = true;
}
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,$%02x,$%02x", thread.thread_id, thread.current_pc, "reset", begin, end, state);
#endif
    switch(state) {
        case 0:
        case 1:
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "loadres", id);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%04x", thread.thread_id, thread.current_pc, "palette", palette);
#endif
...
auto VirtualMachine::op_page(Thread& thread) -> void
{
    const uint8_t dst = _bytecode.fetchByte();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "page", dst);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,$%02x", thread.thread_id, thread.current_pc, "fill", dst, col);
#endif
...
auto VirtualMachine::op_copy(Thread& thread) -> void
{
    ...
    const uint8_t dst = _bytecode.fetchByte();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,$%02x", thread.thread_id, thread.current_pc, "copy", dst, src);
#endif
...
auto VirtualMachine::op_blit(Thread& thread) -> void
{
    const uint8_t src = _bytecode.fetchByte();
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "blit", src);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%03x,%d,%d,%d", thread.thread_id, thread.current_pc, "print", string, text_x, text_y, color);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,%d,%d,%d", thread.thread_id, thread.current_pc, "sound", sound_id, frequency, volume, channel);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x,%d,%d", thread.thread_id, thread.current_pc, "music", music_id, delay, position);
#endif
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s (buffer: %d, offset: %d, x: %d, y: %d, zoom: %d)", thread.thread_id, thread.current_pc, "poly1", polygonDataIndex, offset, poly_x, poly_y, zoom);
#endif
    _engine.drawPolygon(buffer, offset, zoom, get_point(poly_x, poly_y));
}
...
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s (buffer: %d, offset: %d, x: %d, y: %d, zoom: %d)", thread.thread_id, thread.current_pc, "poly2", polygonDataIndex, offset, poly_x, poly_y, zoom);
#endif
    _engine.drawPolygon(buffer, offset, zoom, get_point(poly_x, poly_y));
}
...
{
#ifndef NDEBUG
    log_debug(SYS_VM, "|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "invalid", thread.opcode);
#endif

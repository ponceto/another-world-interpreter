...
{
    log_debug(SYS_RESOURCES, "starting...");
    log_debug(SYS_RESOURCES, "buffer size is %luK", (sizeof(_buffer) / 1024));
    loadMemList();
    log_debug(SYS_RESOURCES, "started!");
}

auto Resources::reset() -> void
{
    log_debug(SYS_RESOURCES, "resetting...");
    log_debug(SYS_RESOURCES, "reset!");
}

auto Resources::stop() -> void
{
    log_debug(SYS_RESOURCES, "stopping...");
    invalidateAll();
    log_debug(SYS_RESOURCES, "stopped!");
}
...
auto Resources::loadPart(uint16_t partId) -> void
{
    ...
    if((partId >= GAME_PART_FIRST) && (partId <= GAME_PART_LAST)) {
        ...
        log_debug(SYS_RESOURCES, "load part [id: 0x%04x, name: '%s']", partId, part->name);
        _curPartId = partId;
        _reqPartId = 0;
    }
    else {
...
auto Resources::loadResource(uint16_t resourceId) -> void
{
    if(resourceId >= countof(_resourcesArray)) {
        log_debug(SYS_RESOURCES, "load resource [part: 0x%04x]", resourceId);
...
{
    log_debug(SYS_RESOURCES, "load memlist");
...
auto Resources::loadResources() -> void
{
    ...
    for(auto& resource : _resourcesArray) {
        ...
        if(resource.state == RS_NEEDED) {
            log_debug(SYS_RESOURCES, "load resource [resource: 0x%02x, bank: 0x%02x]", resource.id, resource.bankId);
    ...
    log_debug(SYS_RESOURCES, "resources memory [consumed: %luK, remaining: %luK]", ((_bufptr - _buffer) / 1024), ((_bufend - _bufptr) / 1024));
}
...
auto Resources::invalidateAll() -> void
{
    for(auto& resource : _resourcesArray) {
        ...
        log_debug(SYS_RESOURCES, "invalidate resource [resource: 0x%02x]", resource.id);
        resource.state = RS_NOT_NEEDED;
        resource.data  = nullptr;
    }
    _bufptr = _buffer;

            ...
            if(resources != nullptr) {
                ...
                while(true) {
                    if(file.read(buffer, 20) != false) {
                        Ptr ptr(buffer);
                        resource->id           = index++;
                        resource->state        = ptr.fetchByte();
                        resource->type         = ptr.fetchByte();
                        resource->unused1      = ptr.fetchWordBE();
                        resource->unused2      = ptr.fetchWordBE();
                        resource->unused3      = ptr.fetchByte();
                        resource->bankId       = ptr.fetchByte();
                        resource->bankOffset   = ptr.fetchLongBE();
                        resource->unused4      = ptr.fetchWordBE();
                        resource->packedSize   = ptr.fetchWordBE();
                        resource->unused5      = ptr.fetchWordBE();
                        resource->unpackedSize = ptr.fetchWordBE();
...
auto MemList::dumpMemList(Resource* resources) -> bool
{
    ...
    {
        log_debug(SYS_RESOURCES, "| 0x%02x | %-8s | %7d bytes | %7d bytes | %6.2f%% |", resource.id, type, resource.packedSize, resource.unpackedSize, gain(resource.packedSize, resource.unpackedSize));
    };
